...
foreach (var rel in files.Where(f => f.EndsWith(".cs", StringComparison.OrdinalIgnoreCase))) {
  ...
  calls.AddRange(GrpcCallsExtractor.Extract(text, rel));
}
var envelope = new Dictionary<string, object?> { ["parser_id"]="dotnet-grpc-calls", ["schema_version"]="1", ["project_id"]=argsMap["project-id"], ["analysis_run_id"]=argsMap["analysis-run-id"], ["generated_at"]=DateTime.UtcNow.ToString("O"), ["files_analyzed"]=files, ["model"]=new Dictionary<string, object?> { ["calls"] = calls } };
