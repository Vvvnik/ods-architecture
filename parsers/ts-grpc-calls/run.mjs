...
for (const p of files) {
  ...
  if (!/\.(ts|tsx|js|jsx|mjs|cjs)$/i.test(rel) || isTestOrSpecPath(rel)) continue;
  const text = await readFile(join(args['working-copy-root'], rel), 'utf8');
  calls.push(...extractTsGrpcCalls(text, rel));
}
await writeFile(args.output, JSON.stringify({ parser_id:'ts-grpc-calls', schema_version:'1', project_id:args['project-id'], analysis_run_id:args['analysis-run-id'], generated_at:new Date().toISOString(), files_analyzed:files, model:{calls}}, null, 2));
