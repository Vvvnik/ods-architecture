...
| `ERR_MODULE_NOT_FOUND` / граф не строится после clone | Пересобрать backend из корня: `docker compose -f docker/docker-compose.dev.yml --profile full up --build -d` (парсеры в образе, без mount с хоста) |
