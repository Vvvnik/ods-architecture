...
export function resolveBindsServiceTarget(app: NativeApp): string | null {
  ...
  if (!serviceName) {
    ...
    for (const part of entryPath.split('/').filter(Boolean)) {
      ...
      if (
        ['src', 'main', 'resources', 'static', 'scripts', 'app', 'apps'].includes(lower) ||
        part.includes('.')
      ) {
        continue;
      }
    ...
    serviceName =
      [...candidates].find((c) => c.includes('gateway')) ??
      [...candidates].find((c) => c === 'ui' || c === 'web') ??
      [...candidates][0] ??
      null;
  }
...
export const angularjsUiIngestAdapter: IngestAdapter = {
