...
export class AgentPromptService {
  ...
  /** Seed AGENT.md if missing (existing projects imported before 015). */
  ...
  private async readTemplate(): Promise<string> {
    const configured = this.config.DOCS_PROMPT_TEMPLATE;
    const candidates = [
      resolve(configured),
      resolve(process.cwd(), configured),
      resolve(this.config.PARSERS_ROOT, '..', 'prompts', 'docs-agent-prompt.md'),
      resolve(process.cwd(), 'prompts', 'docs-agent-prompt.md'),
    ];
    const errors: string[] = [];
    ...
    throw new Error(`Docs prompt template not found. Tried: ${errors.join(', ')}`);
  }
}
