...
import { cpSync, mkdtempSync, readFileSync, rmSync, readdirSync, statSync } from 'node:fs';
...
function listProductionJava(root: string): string[] {
  const out: string[] = [];
  const walk = (dir: string) => {
    for (const name of readdirSync(dir)) {
      ...
      if (rel.includes('/src/main/java/') && rel.endsWith('.java')) {
        out.push(rel);
      }
    }
  };
...
describe('java parser calls (023)', () => {
  let repoRoot: string;
  let outputPath: string;

  beforeEach(() => {
    repoRoot = mkdtempSync(join(tmpdir(), 'ods-java-calls-'));
    outputPath = join(repoRoot, 'envelope.json');
    cpSync(join(process.cwd(), '../docker/fixtures/repos/java-calls-demo'), repoRoot, {
      recursive: true,
    });

    const runSh = join(process.cwd(), '../parsers/java/run.sh');
    const files = listProductionJava(repoRoot);
    execFileSync(
      'bash',
      [
        runSh,
  ...
  it('emits schema_version 2 with instance/static/cross-module/interface calls', () => {
    ...
    expect(envelope.model.symbols?.some((s) => s.kind === 'method')).toBe(true);
    const usages = envelope.model.usages ?? [];
    expect(usages.some((u) => u.type === 'calls' && u.from.includes('Service.create') && u.to.includes('Service.save'))).toBe(
      true,
    );
    expect(usages.some((u) => u.type === 'calls' && u.to.includes('Utils.stamp'))).toBe(true);
    expect(usages.some((u) => u.type === 'calls' && u.to.includes('BetaHelper.help'))).toBe(true);
    expect(
      usages.some((u) => u.type === 'calls' && u.to.includes('Clock.now') && !u.to.includes('SystemClock')),
    ).toBe(true);
    expect(usages.some((u) => u.from.includes('Overloads.run'))).toBe(false);
  });

  it.skipIf(!esAvailable)('ingests calls edge into elasticsearch (SC-004 API)', async () => {
    ...
    ingestRegistry.register(javaIngestAdapter);
    ...
    await analysisRunRepository.create({
      ...
      language_report_id: '00000000-0000-4000-8000-000000000055',
      status: 'success',
